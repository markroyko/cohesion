...
class LayoutCanvas implements LayoutCanvasElementInterface, \JsonSerializable {
  ...
  public function __construct($json_values) {
    ...
    $model = property_exists($this->raw_decoded_canvas, 'model') && is_object($this->raw_decoded_canvas->model) ? $this->raw_decoded_canvas->model : FALSE;

    if (property_exists($this->raw_decoded_canvas, 'previewModel')) {
      $this->previewModel = $this->raw_decoded_canvas->previewModel;
    }

    if (property_exists($this->raw_decoded_canvas, 'variableFields')) {
      $this->variableFields = $this->raw_decoded_canvas->variableFields;
    }

    if (property_exists($this->raw_decoded_canvas, 'mapper')) {
      $this->mapper = $this->raw_decoded_canvas->mapper;
    }

    if (property_exists($this->raw_decoded_canvas, 'canvas') && is_array($this->raw_decoded_canvas->canvas)) {
      foreach ($this->raw_decoded_canvas->canvas as $index => $element) {
        $this->canvasElements[$index] = new Element($element, $this, $model);
      }
    }

    if (property_exists($this->raw_decoded_canvas, 'componentForm')) {
      $this->componentFormElements = [];
      if (is_array($this->raw_decoded_canvas->componentForm)) {
        foreach ($this->raw_decoded_canvas->componentForm as $index => $element) {
          $this->componentFormElements[$index] = new Element($element, $this, $model);
        }
      }
    }

    if (property_exists($this->raw_decoded_canvas, 'styleGuideForm')) {
      $this->styleGuideFormElements = [];
      if (is_array($this->raw_decoded_canvas->styleGuideForm)) {
        foreach ($this->raw_decoded_canvas->styleGuideForm as $index => $element) {
          $this->styleGuideFormElements[$index] = new Element($element, $this, $model);
        }
      }
    }

    if (property_exists($this->raw_decoded_canvas, 'disabledNodes')) {
      $this->disabledNodes = $this->raw_decoded_canvas->disabledNodes;
    }

    if (property_exists($this->raw_decoded_canvas, 'meta')) {
      $this->meta = $this->raw_decoded_canvas->meta;
    }
  }
